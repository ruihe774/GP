...
- `session.py` — main event loop. Holds `AsyncRealtimeConnection` and drives the API. Accumulates gamepad summaries and chooses at most one frame per response. Replaces the expensive half of the conversation with the cheap version of itself (`_maybe_prune` on a timer, `_prune_now` when the window is the problem). Implements barge-in (cancel response, truncate audio) on player speech. Listens for `response.done`, computes cost and usage, and answers the server's refusal when the conversation no longer fits (`_on_context_full`).
...
- `persona.py` — agent personality and instructions, shaped to OpenAI's realtime prompting guide for the `gpt-realtime-2` family: short labelled sections, a stated priority for when two rules collide, and explicit `# Preambles` / `# Variety` sections because those are default behaviours of the model rather than things it needs asking for. Also holds `WAIT_TOOL` (`wait_for_user`) and the `WAIT_NOTE` that goes with it — the tool description says what it is, the note says using it is allowed.
...
- Pruning removes items on **one shared cutoff, in one round**, and it **replaces rather than deletes**. Both halves of the first part are about the prompt cache: one cutoff because things created together should go together, and rounds because a delete truncates the cached prefix at the deleted item — and removals come from the oldest end, so any round costs essentially the whole prefix. That price is per round, so `prune_interval_s` is the knob that decides what pruning costs. Four kinds go: `image` (a screenshot item becomes `[dropped] N frames of what was on screen here.`), `heard` (the player's committed utterance becomes its transcription), `said` (the agent's own spoken turn becomes an assistant `output_text` item carrying what it said), and `note` — the only one deleted outright, and the only one taken **regardless of age**, since a per-turn nudge stopped being true the moment its turn ended. Sweeping every nudge is not the same as *triggering* on one: the trigger is still "something past the cutoff", or a round would fire every `prune_interval_s` forever to save thirty tokens. Each replacement is `conversation.item.create` with `previous_item_id` set to the victim, **then** the delete — never the reverse, because `previous_item_id` must name a live item, and inserting after the victim is what keeps a multi-victim round in order with no bookkeeping. The insert is free in cache terms: the round's ceiling is already fixed by its earliest delete, so stubs cost tokens (a dozen each, permanently) and not a second invalidation. `agent.prune_after_s = 0` turns off only the *scheduled* round; a forced one (see the cost model) ignores it.
- Server-side truncation is **disabled** (`agent.truncation`), which makes the client the only thing standing between a long session and the context window. That is deliberate and it is about one item: the server trims from the oldest end, and the oldest end is the subtitle script — the one thing the session cannot reconstruct, put first precisely so it is cached. Nothing tells the client it was trimmed, so the failure would be an agent that has quietly stopped knowing what is being said. Disabled, the server refuses the response instead, which is a failure the client can answer: `context_budget_tokens` forces a round when a response bills past it (before the ceiling), `_on_context_full` forces one when the server refuses (after it), and a refused turn is re-asked at most `context_full_retries` times. A forced round climbs three rungs — the scheduled cutoff, then everything but the turn in flight, then the stubs themselves keeping the newest `context_full_keep_stubs` — and if none of them frees anything the transport is closed so the pump reopens the session, the same recovery the API's 60-minute cap already gets. Nothing a round takes belongs to the turn being answered (`_live_items`), because freeing room by deleting the question is asking a different question.
- The model is allowed to decline a turn, and it needs a tool to do it with. `SpeakPolicy` decides a moment is worth a remark from what capture can see, which cannot tell the player's voice from the television's or a burst that meant something from one that did not; the model can, and `wait_for_user` (`agent.wait_tool`, on by default) is the only way it has to say so. A realtime model with no such action fills the turn — "I'm here", "take your time" — which is exactly the failure OpenAI's guide names. The reply to the call is a `function_call_output` and deliberately **not** another `response.create`: asking again is asking the same question that already came back "nothing to say". Both items are registered `wait`-kind disposables, deleted outright on the next round like a nudge, and the turn is not recorded as an `agent.response` — it cost a request and a slot under the rate cap, which is what `report()["held"]` counts, but nobody heard it.
- Preambles are stopped twice. `gpt-realtime-2` emits them by default (a short spoken update before the real answer) and this product has no use for one: nothing is being looked up and nobody is waiting. The persona says so, and a response part the server marks `phase: "commentary"` is dropped client-side — not played, not shown on the HUD, not folded into the transcript, though the item stays in the conversation because the model believes it said it. Note that `phase` is documented on `response.done` output parts; reading it off `response.output_item.added` (which is where it has to be read, since audio streams) is not, so the prompt is the load-bearing half and the gate is the net.
- `agent.reasoning_effort` defaults to `"low"`, not to the server's default, on the guide's advice: this agent produces one sentence of reaction with no tools to orchestrate, and the cost of thinking longer is not tokens but a remark that lands after the moment it was about. Non-reasoning models (`gpt-realtime`, `gpt-realtime-1.5`) reject the field — `--set agent.reasoning_effort=null` omits it.
...
- `src/gpagent/config.py` (capture) and `src/gpagent/agent/config.py` (agent) hold defaults in code. TOML sections `[gamepad] [audio] [screen] [triggers]` are Component A; `[agent] [speak] [hud] [subtitles]` are Component B. `agent.truncation_retention_ratio` is kept but inert unless `agent.truncation = "retention_ratio"`: removing the key would turn every existing config and recorded manifest into an error rather than a migration. Per-device gamepad overrides go under `[gamepad.device_button_map."<vid:pid>"]`.
...
Subtitles are the one input that is cheap *because* the cache exists: the whole film's dialogue, sent once ahead of the first turn, is fresh input once and cached thereafter (~12% on a sess_movie2 dry run), where the same text streamed cue-by-cue would be uncached every turn. Output audio (speaking) is the dominant cost line and is controlled entirely by the speak policy's rate cap. `agent.output = "text"` deletes that line outright — 14 remarks over a sess5 replay billed 478 output text tokens for $0.0011, against roughly a cent of speech for the same fourteen — which leaves re-billed input as effectively the whole bill. Input images are the next lever: cost is per-frame, not per-pixel-stream, because Component B sends at most one high-detail frame per response regardless of how many Component A captured — capture rate and send rate are fully decoupled. Cached input tokens are far cheaper than fresh ones (prompt caching), which is why trimming for *cost* loses: deleting old items invalidates the cache prefix ahead of them, so it can raise the bill even though it shrinks the conversation (sess5, keeping every image: 31008 image tokens for $0.0204; keeping the last two: 8721 for $0.0268). The server's `truncation.retention_ratio` is the sanctioned mechanism and it is *turned off here* — it drops in amortized batches that don't disturb the prefix, and it drops from the oldest end, where the script is (see the design patterns above). `agent.prune_after_s` is the client-side one, off by default, and it is a TPM instrument rather than a cost one.

What changes the arithmetic is that a round now *replaces* what it removes rather than deleting it. The cache still dies once per round — that price is unchanged — but the conversation keeps its thread, and two lines that used to be uncontrollable become controllable. The player's committed utterance and the agent's own spoken reply are named by the server, so before this the client had no id for them and audio was a permanent floor under every request, re-billed at 32 $/Mtok (4 on `-mini` against 0.6 for text) for the rest of the session; both now come back as their transcripts. Audio input is 8x text on the flagship (32 $/Mtok against 4) and ~17x on `-mini` (10 against 0.6). On a sess5 dry run with a 30 s cutoff that is $0.1350 → $0.0690 for the same fourteen responses; on sess_movie2 under `movies.example.toml`, $0.8141 → $0.3294 and 772k image tokens → 310k, with the peak request and worst 60 s window unchanged from the numbers below (the images were already going). The same run reads $0.3374 today: the persona grew by the preamble, variety and `wait_for_user` sections, and the persona is the one item billed on every single request. That is what a paragraph of instructions costs over 36 responses — under 3%, and the reason prompt changes get measured rather than argued about. Against that, the stubs are permanent: a dozen tokens each, one per removed item, re-billed forever. Read them off the dry run rather than trusting the estimate. Exact per-token rates live in `tokens.py::MODEL_RATES`, not here.
...
**Measuring what history costs:** `commentate --dry-run` bills the whole conversation on every response, not just the turn's own items — `RecordingTransport` keeps a ledger of live items, `conversation.item.delete` removes from it, `previous_item_id` inserts into the middle of it, and cached tokens are modelled by the same prefix rule the server uses. So the growth curve, and what a pruning setting does to it, can be read offline: `commentate --replay sess_movie2 --dry-run -c movies.example.toml` against `--set agent.prune_after_s=0`. On sess_movie2 that is a peak request of 15.6k tokens against 46.4k, and a worst 60 s window of 52k against 163k. It is a model of the server, not the server — read the shape, not the fourth digit, and note that server-side truncation is not modelled at all, so a `retention_ratio` dry run is not a prediction of one.

**Exercising the context ceiling offline:** `--dry-run-context-limit N` makes the dry run refuse a `response.create` once the conversation passes N post-instruction tokens, which is what the API does with truncation disabled. `commentate --replay sess5 --dry-run --dry-run-context-limit 4000` drives the refusal, the forced round and the retry; a limit small enough that one turn cannot fit drives the last rung and the reopen instead. Under `--replay-speed 0` add `--set agent.reconnect_min_s=0`, or the reconnect backoff sleeps in real time while the replay finishes without it.
...
- `previous_item_id` must name an item that is still in the conversation — an id the server cannot find is `item_create_invalid_previous_item_id` ("Error adding item: the previous item with id … does not exist", confirmed live) and the item is simply not added. That is why a pruning round creates the replacement *first* and deletes second, never the other way round, and why it inserts after the victim rather than after the victim's predecessor (which for server-named audio items the client never learns).
- `conversation.item.create` cannot populate assistant **audio** — the API says so outright. A pruned spoken reply therefore goes back as an assistant message with `output_text` content, which is also the cheaper half of the trade.
- The error code for "the conversation is too long to create a Response" is **not documented anywhere**, so `_is_context_full` matches on code substrings first and falls back to a message match that requires both a size word and a word naming the conversation; `rate_limit*` is excluded by name because it is the closest miss (also about size, but of a request window, and it wants backoff). `agent.context_full_codes` is the escape hatch. The same refusal arrives as an `error` event, or as a failed `response.done`, or both — `_retry_pending` collapses them.
- A refused `response.create` never produces a `response.done`, and `_speak` sets `_response_active` optimistically, so anything that gives up on a turn must release that gate itself (`_abandon_turn`) or the agent goes quiet for `speak.response_gate_timeout_s`. For the same reason, `_turn_in_flight` bounds the "do not prune the live turn" guard by that timeout: an absolute gate that can never clear turns one lost event into a conversation that can never be pruned again.
- Audio is never removed before its transcript is known, because deleting it takes the only record of what was said. The transcript arrives on a later event (`conversation.item.input_audio_transcription.completed` for the player, `response.output_audio_transcript.done` for the agent), so entries are registered `pending` and settled afterwards — with `agent.audio_stub_wait_s` as the deadline, and a settle on cancel, so a lost transcription cannot pin an item forever. With `transcribe_player = false` the player's audio gets the generic stub immediately instead.
- `input_audio_buffer.committed` is the *only* place the id of the player's committed utterance is ever stated. Without handling it that item is unremovable for the life of the session, which is what made audio a permanent floor under the request.
...
- A response can produce more than one text part — sess_movie3 had one of thirty-two come back as a preamble plus the remark, two `response.output_text.done` events 0.01 s apart. Each part is shown and logged as it arrives; `_add_transcript` joins them so the recorded `agent.response` is what the viewer actually read. (That preamble is the behaviour the `# Preambles` section and the `phase: "commentary"` gate now exist for; parts that survive the gate are still joined, since a response legitimately split in two is not a preamble.) Nothing here ever carries reasoning: `response.output_text.done` is an assistant *content part*, and this SDK has no reasoning-text event at all.
- `phase` is a declared field in the installed SDK (`openai` 2.53.0) — `Literal["commentary", "final_answer"]` on `types/responses/response_output_message.py` and its `beta`/`conversations` twins — but **not** anywhere under `types/realtime/`, where the item arrives as `RealtimeConversationItemAssistantMessage`. It reaches the agent anyway, and that is the transport working as designed: incoming events are `model_dump()`ed off pydantic models configured `extra="allow"`, so a server field this SDK's realtime types have never heard of survives the round trip. Verified against `construct_type(..., RealtimeServerEvent)`, not assumed. Outgoing payloads have no such luck — `send()` drops unknown keys — which is what `TestSdkPayloadDrift` exists for.
- Nothing offline exercises the hold path: `RecordingTransport` never emits a `function_call`, so a dry run cannot show the model declining a turn. Same shape as the text-output gotcha below — the fake can answer, but it cannot decide not to.
